...
```
"""
function mask_area(geometry::CoordinateSystem, mask::Array{T, 2}, res = 15) where T<:Real
...
```
load_example()
mask = polygon_mask(mumbai_map[1, :].geometry)
mask_area(INDIA_COORDINATE_SYSTEM, mask) 
```
"""
function mask_area(geometry::CoordinateSystem, mask::Mask, res = 15)
    mask_area(geometry, mask.vals, res)
end