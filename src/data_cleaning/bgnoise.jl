...
```
"""
function bgnoise_PSTT2021(radiance_datacube, clouds_datacube; threshold = 0.4)
    function noise_threshold(x,threshold = 0.4)
        if x<=threshold
            return missing
        else
            return 1
    ...
    mask = noise_threshold.(average_lastyear, threshold)
